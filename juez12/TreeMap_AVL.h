...
public:
   ...
   void print(std::ostream& o = std::cout) const {
    ...
    // Metodos en el que se toca la implementar

   std::vector<int> rangeToValues(int min, int max){
      std::vector<int> vc;
      range(min, max, vc, raiz);
      return vc;
   }

private:

   void range(int const min, int const max, std::vector<int> & vc, Link a){

       if (a == nullptr) {
            return;
       }
       // Si la raiz >= min
       else if(!(a->cv.clave < min)){
            range(min, max, vc, a->iz);
            if(!(max < a->cv.clave)){
                vc.push_back(a->cv.clave);
                range(min, max, vc, a->dr);
            }
       }else if(!(max < a->cv.clave)){
           range(min, max, vc, a->dr);
       }
   }
